...
def train_net(cfg):
    ...
    basenet_list={'volleyball':Basenet_volleyball, 'collective':Basenet_collective}  # 第一阶段的网络不变
    gcnnet_list={'volleyball':GCNnet_volleyball,
                 # 'at_volleyball': AT_volleyball,  # 第二阶段需要添加AT
    ...
    if cfg.training_stage==1:
        ...
        GCNnet=gcnnet_list[cfg.dataset_name]  # 修改推理网络的选项
        model=GCNnet(cfg)
        # Load backbone
    ...
    for epoch in range(start_epoch, start_epoch+cfg.max_epoch):
        ...
        if epoch % cfg.test_interval_epoch == 0:
            ...
            if cfg.training_stage==2:
                # None
                if test_info['activities_acc'] > 91:  # set threshold
                    state = {
                        'epoch': epoch,
...
def train_volleyball(data_loader, model, device, optimizer, epoch, cfg):
    ...
    for batch_data in data_loader:
        ...
        actions_in=batch_data[2].reshape((batch_size,num_frames,cfg.num_boxes))  # （8 1 12）
        activities_in=batch_data[3].reshape((batch_size,num_frames))  # （8 1）

        actions_in=actions_in[:,0,:].reshape((batch_size*cfg.num_boxes,))  # （8 1 12）-》（96，）
        activities_in=activities_in[:,0].reshape((batch_size,))  # （8 1）-》（8，）

        # forward  batch_data[0]：（8 1 3 720 480） batch_data[1]：（8 1 12 4）
        actions_scores,activities_scores=model((batch_data[0],batch_data[1]))
        # actions_scores(384,9) activities_scores(32,8)
        # Predict actions
        actions_weights=torch.tensor(cfg.actions_weights).to(device=device)  # actions_weights有何作用
        actions_loss=F.cross_entropy(actions_scores,actions_in,weight=actions_weights)  # actions_scores(384,9) actions_in(384,)
        # out是网络输出的概率向量，y是真实标签，注意y是标量。使用这个函数时应该特别注意out是没有经过softmax处理过的。
        # 因为调用F.cross_entropy函数时会通过log_softmax和nll_loss来计算损失，  actions_loss=10.5309 标量
        # 也就是说使用F.cross_entropy函数时，程序会自动先对out进行softmax，再log，最后再计算nll_loss。
        # weight=actions_weights表示9个个体行为的权重  总结：当我们增加少数样本的权重时，计算出的损失值应该是放大的。 weight就是为每一个样本加权。
        actions_labels=torch.argmax(actions_scores,dim=1)  # 返回指定维度最大值的序号  actions_labels（384，）
        actions_correct=torch.sum(torch.eq(actions_labels.int(),actions_in.int()).float())  # 预测正确的数量 actions_correct=177 标量

        # Predict activities
        activities_loss=F.cross_entropy(activities_scores,activities_in)   # 没带weight=actions_weights权重
